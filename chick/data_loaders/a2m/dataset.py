...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.use_action_cat_as_text_labels = kwargs.get(
    ...
    def action_name_to_action(self, action_name):
        ...
        if isinstance(all_action_names, dict):
            ...
            assert list(self._action_classes.keys()) == list(
                range(len(all_action_names))
        ...
        if pose_rep != "xyz":
            if getattr(self, "_load_rotvec", None) is None:
                ...
                if self.align_pose_frontview:
                    first_frame_root_pose_matrix = geometry.axis_angle_to_matrix(
                        pose[0][0]
                    )
                    ...
                    pose[:, 0, :] = geometry.matrix_to_axis_angle(
                        aligned_root_poses_matrix
                    )

                    if self.translation:
                        ret_tr = torch.matmul(
                            torch.transpose(first_frame_root_pose_matrix, 0, 1).float(),
