...
class TransformerUNet(nn.Module):
    """
    A Transformer based U-Net model with long range skip connections.

    Each layer in the encoder is connected to the layer of the same resolution in the decoder
    i.e. The first layer is connected to the last layer and so on.

    e.g. for a 4 layer encoder and 4 layer decoder, the connections are as follows:
    - 1st and 4th
    - 2nd and 3rd
    """
    ...
    def __init__(self, encoder_layer, num_layers):
        ...
        n_encoder_layers = num_layers // 2
        n_decoder_layers = num_layers // 2
        n_middle_layer = num_layers - n_encoder_layers - n_decoder_layers

        self.encoder = _get_clones(encoder_layer, num_layers)
        self.decoder = _get_clones(encoder_layer, num_layers)

        if n_middle_layer > 0:
            self.middle = _get_clones(encoder_layer, n_middle_layer)
...
class Decoder_TRANSFORMER(nn.Module):
    ...
    def forward(self, batch):
        ...
        z = z.unsqueeze(0).repeat(nframes, 1, 1)
...
class MartinezTransformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        super(MartinezTransformerEncoderLayer, self).__init__()
    ...
    def __setstate__(self, state):
        super(MartinezTransformerEncoderLayer, self).__setstate__(state)
