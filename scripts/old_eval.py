...
model_path = {
    ...
    49: "./model_30_frames.pt",
    99: "./model_30_frames.pt",
}[n_frames]
...
if __name__ == "__main__":
    ...
    for k, (
        ...
        def energy_fn(x, plot=False):
            ...
            energy = (
                ((x_2D_projected - input_2D.permute(0, 3, 1, 2)) ** 2)
                .mean(-1)
                .mean(-1)
                .mean(-1)
            )
            ...
            return {
                "train": energy,
                "input_2D": input_2D
                + center.unsqueeze(0).permute(0, 1, 3, 2)[..., [0, 1], :],
            }
        ...
        mms.append(np.nanmin(m.cpu().numpy()))
        # mms.append(np.nanmean(m.cpu().numpy()))
        ...
        pbar.set_description(
            f"{mms[-1]:.2f} | {np.nanmean(mms):.2f} | {_calibration_score:.2f}"
        )
        ...
        # print("gt_3D_projected.shape", gt_3D_projected.shape, "input_2D.shape", input_2D.shape, "sample_2D_proj.shape", sample_2D_proj.shape)
        # plot_2D(
        ...
        # print("gt_3D.shape", gt_3D.shape, "input_2D.shape", sample.shape, "sample_2D_proj.shape")
        print(sample.shape)

        # for each frame choose the best sample using idx
        sample = sample.permute(0, 1, 3, 2)
        # best_sample = torch.stack([sample[idx[i], ..., i] for i in range(n_frames)], dim=-1)
        # print("best_sample.shape", best_sample.shape)

        plot_3D(
            gt_3D.permute(0, 3, 2, 1),
            # best_sample[None],
            sample.permute(0, 1, 3, 2)[idx][None],
